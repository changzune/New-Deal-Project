...
<c:if test="${empGradeTotalCnt > 0 }">
	...
	<c:forEach items="${listEmployeeModel}" var="list">
		...
		<td>${list.promotion_date}</td>
		 <td><a href="javascript:empDtlGrade(1, '${list.loginID }')"><strong>${list.loginID}</strong></a></td>
         <td><a href="javascript:viewEmpDtl('${list.loginID }')"> <strong>${list.name}</strong></a></td>
