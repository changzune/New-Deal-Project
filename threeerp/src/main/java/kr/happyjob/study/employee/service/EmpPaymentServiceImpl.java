...
public class EmpPaymentServiceImpl implements EmpPaymentService {
	...
	public List<EmployeeModel> empPaymentList(Map<String, Object> paramMap) {
		...
		String str = (String)paramMap.get("from_date"); //클라이언트에서 가져온 날짜값을 "-" 기준으로 두개의 배열로 저장
		String[] strAry = str.split("-");
		int from_date_year = Integer.parseInt(strAry[0]); // 배열중 첫번째인 년도를 저장 
		int from_date_month = Integer.parseInt(strAry[1]); // 배열중 두번째인 월을 저장 
		
		int flag;
		// 클라이언트에서 요청한 년도 값이 현재 년도보다 같거나 크고 요청한 월이 현제 월보다 크거나 같을 때
		if(year <= from_date_year && month <= from_date_month) { //tb_emp(현재 연봉)으로 조회
			flag = 1;
			System.out.println("flag == 1");
			paramMap.put("flag", flag);
			// 클라이언트에서 요청한 년도 값이 현재 년도보다 작을경우,혹은  요청한 년도가 같지만 년도가 작은 경우
		}else if (year > from_date_year || (year == from_date_year && month > from_date_month)){ //tb_sal_his(이미 지급된 내역)으로 조회
			flag = 2;
